...
//        }
        return this.customers.stream().map(Customer::getOrders).flatMap(Collection::stream).toList();
    }
    ...
    {
        Customer find = this.customers.stream().filter(customer -> customer.getName().equals(name)).findFirst().orElse(null);
        return find;
    }
}
