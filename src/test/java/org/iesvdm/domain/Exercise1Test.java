...
public class Exercise1Test extends CompanyDomainForKata
{
    ...
    public void getCustomerNames()
    {
        ...
        List<String> customerNames = this.company.getCustomers().stream().map(nameFunction).toList();
    ...
    {
        List<String> customersFromLondon = this.company.getCustomers().stream().filter(customer -> customer.getCity().equals("London")).map(Customer::getName).toList();
